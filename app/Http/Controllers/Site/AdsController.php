...
class AdsController extends Controller
{
    ...
    public function __construct(Ads $ads, AdsCategory $adsCategory, City $city, ReportType $reportType, Country $country, AdsAttributeDescription $attributeDescription)
    {
        ...
        $this->indexPageFields = [
            'id', 'category_id', 'city_id', 'user_id', 'title', 'phone', 'created_at'
        ];
    ...
    private function getAdsIndexByCategory($id, $title)
    {
        ...
        return view('v1.site.pages.ads.all-index', compact('ads', 'page', 'category'));
    }
    ...
    private function getAdsIndexByCity($id, $title)
    {
        ...
        return view('v1.site.pages.ads.all-index', compact('ads', 'page'));
    }
    ...
    public function store(AdsCategory $adsCategory, AdsStoreRequest $request)
    {
        try {
            ...
            DB::commit();
            session()->flash('notifications', ['message' => trans('site/messages.adsCreateSuccess'), 'alert_type' => 'success']);
    ...
    public function update(Ads $ads, AdsUpdateRequest $request)
    {
        try {
            ...
            session()->flash('notifications', ['message' => trans('site/messages.adsUpdateSuccess'), 'alert_type' => 'success']);
            return redirect()->route('ads.show', [$ads->id, $ads->urlTitle]);
        } catch (\Exception $e) {
            session()->flash('notifications', ['message' => trans('site/messages.operationFailed'), 'alert_type' => 'error']);
            return redirect()->back();
