                    ...
                    <div class="category-widget">
                        <ul>
                            @foreach($blogCategories as $category)
                                <li class="arrow-list4"><a href="{{ route('blog.category-blog', ['blogCategory' => $category->id, 'title' => $category->urlTitle]) }}">{{ $category->shortTitle }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                            ...
                            @foreach($topLikes as $row)
                                <div class="post-single">
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('blog.show', ['blog' => $row->id, 'title' => $row->title]) }}"><img src="{{ $row->logo['small'] }}" alt="{{$row->title}}" title="{{ $row->title }}"></a>
                                        <p><span>{{ $row->jalali_published_at }}</span> <span>توسط <a href="#">{{$row->admin->full_name}}</a></span></p>
                                    </div>
                                    <a href="{{ route('blog.show', ['blog' => $row->id, 'title' => $row->urlTitle]) }}">{{ $row->shortTitle }}</a>
                    ...
                    <div class="widget-content">
                        <div class="sidebar-post">
                            @foreach($topViews as $row)
                                <div class="post-single">
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('blog.show', ['blog' => $row->id, 'title' => $row->urlTitle]) }}"><img src="{{ $row->logo['small'] }}" alt="{{$row->title}}" title="{{ $row->shortTitle }}"></a>
                                        <p><span>{{ $row->jalali_published_at }}</span> <span>توسط <a href="#">{{$row->admin->full_name}}</a></span></p>
                                    </div>
                                    <a href="{{ route('blog.show', ['blog' => $row->id, 'title' =>$row->urlTitle]) }}">{{ $row->shortTitle }}</a>
                                </div><!-- ends: .post-single -->
                    ...
                    <div class="widget-content">
                        <div class="sidebar-post">
                            @foreach($newBlogs as $row)
                                <div class="post-single">
                                    <div class="d-flex align-items-center">
                                        <a href="{{ route('blog.show', ['blog' => $row->id, 'title' => $row->urlTitle]) }}"><img src="{{ $row->logo['small'] }}" alt="{{$row->title}}" title="{{ $row->title }}"></a>
                                        <p><span>{{ $row->jalali_published_at }}</span> <span>توسط <a href="#">{{$row->admin->full_name}}</a></span></p>
                                    </div>
                                    <a href="{{ route('blog.show', ['blog' => $row->id, 'title' => $row->urlTitle]) }}" >{{ $row->shortTitle }}</a>
                                </div><!-- ends: .post-single -->
