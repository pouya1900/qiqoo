            ...
            <td id="column0" class="connectcolumn" colspan="2">
                ...
                <div class="panel" id="basic">
                    ...
                    <div class="panel-body">
                        ...
                        <a href="{{route('admin.unit.create')}}" data-toggle="popover" class="btn btn-warning well b#52D078" data-placement="top" data-trigger="hover" data-content="برای رکورد جدید کلیک کنید" data-original-title="" title="">رکورد جدید</a>
                            @if($subSequence['id'] != 0) <a href="{{route('admin.unit.index')}}" data-toggle="popover" class="btn btn-warning well b#225278" data-placement="top" data-trigger="hover" data-content="همه ی رکوردها" data-original-title="" title="">همه ی رکوردها</a> @endif
                        </div>
                            ...
                            <div class="panel-body">
                                ...
                                <table id="example" class="table table-condensed table-hover table-striped table-responsive data-table">
                                    ...
                                    @foreach($units as $row)
                                        <tr class="clickable-row" data-url="{{route('admin.unit.edit', $row->id)}}">
                                            <td>{{ $loop->index+1 }}</td>

                                            <td>@if($row->admin->full_name){{$row->admin->fullName}}@else{{$row->admin->username}}@endif</td>
                                            <td>{{  $row->shortTitle }}</td>
                                            <td>{{  $row->jalaliAdminCreatedAt }}</td>
                                            <td>
                                                @include('v1.admin.includes.page-table-buttons',
                                                ['table_edit'=>['route' => 'admin.unit.edit', 'id' => $row->id]],
                                                ['table_delete' => ['model_type' => 'unit', 'id' => $row->id]])
                                            </td>
                                        </tr>
                                    @endforeach
