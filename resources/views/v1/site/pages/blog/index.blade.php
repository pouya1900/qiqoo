					...
					@foreach($blogs as $blog)
						<div class="blog-single">
							<div class="card post--card post--card2 ">
								<figure>
                                    <a href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->title]) }}"><img style="width: 730px; height: 420px" src="{{ $blog->logo['large'] }}"
								...
								<div class="card-body">
									<h3><a href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->title]) }}">{{ $blog->shortTitle }}</a></h3>
									<ul class="post-meta list-unstyled">
										<li>{{ $blog->jalali_published_at }}</li>
										<li>توسط: <a href="#">{{ Str::limit($blog->admin->full_name, 30, '(...)') }}</a></li>
										<li>در دسته: <a href="{{ route('blog.category-blog', ['blogCategory' => $blog->category->id, 'title' => $blog->category->title]) }}">{{ $blog->category->shortTitle }}</a></li>
										<li><a href="#">{{$blog->publishedCommentCount}} نظر</a></li>
										<li><a href="#">{{$blog->viewCount}} بازدید</a></li>
									</ul>
									<p ><a class="blog-description-link" href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->title]) }}" title="{{ $blog->title }}">{{ $blog->short_description }}</a></p>
								</div>
							</div><!-- End: .card -->
