			...
			@foreach($blogs as $blog)
				...
				<div class="grid-single">
					<div class="card post--card shadow-sm">
						<figure>
							<a href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->urlTitle]) }}"><img src="{{ $blog->logo['medium'] }}" alt="{{$blog->title}}" title="{{ $blog->title }}"></a>
						</figure>
						<div class="card-body">
							<h6><a href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->urlTitle]) }}">{{ $blog->shortTitle }}</a></h6>
							...
							<p class="text-justify"><a class="blog-description-link" href="{{ route('blog.show', ['blog' => $blog->id, 'title' => $blog->urlTitle]) }}" title="{{ $blog->title }}">{{ Str::limit($blog->short_description, 100, '(...)') }}</a></p>
						</div>
					</div><!-- End: .card -->
